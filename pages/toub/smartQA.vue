			...
			<view  class="flex-column-start" v-for="(x,i) in msgList" :key="i">
				...
				<view v-if="!x.my" class="flex-row-start margin-left margin-top one-show" >
					<view class="chat-img flex-row-center">
						<image style="height: 75rpx;width: 75rpx;" src="../../static/imgs/robt.png" mode="aspectFit"></image>
					</view>
	...
	export default {	
		...
		data() {
			return {
				...
				type:1,questionList:["如何网上申报","我想了解业务流程","系统管理如何设置"]}],
				msg:"",
				go:0,
				srcollHeight:0
			}
		...
		methods: {
			...
			msgKf(x){				
				...
				setTimeout(()=>{
					// 取消loading
					this.msgLoad=false
					this.msgList.push({my:false,msg:"娜娜还在学习中,没能明白您的问题,您点击下方提交反馈与问题,我们会尽快人工处理(无法回答模板)",type:0,questionList:["如何网上申报","我想了解业务流程","系统管理如何设置"]})
					this.msgList.push({my:false,msg:"单消息模板",type:-1})
					this.msgList.push({my:false,msg:"根据您的问题,已为您匹配了下列问题(多个答案模板)",type:2,questionList:["如何网上申报","我想了解业务流程","系统管理如何设置"]})
					this.msgGo()
				},2000)
			},
			// 不建议输入框聚焦时操作此动画
